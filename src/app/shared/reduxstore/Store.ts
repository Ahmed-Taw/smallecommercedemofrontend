...
export function rootReducer(state: IAppState, action) {
   console.log('reducer');
    switch (action.type) {
        ...
        case ADD_TO_CART:
        let indexofItem = state.cart.shoppingCartItems
                                   .indexOf(state.cart.shoppingCartItems
                                   .filter(i => i.productid === action.cartItem.productid)[0]);
        let modifiedShoppingList;
        if (indexofItem >= 0) {
        const existingItem = state.cart.shoppingCartItems.filter(i => i.productid === action.cartItem.productid)[0];
        ...
        case REMOVE_FROM_CART:
           const existingitem = state.cart.shoppingCartItems
                                          .filter(i => i.productid === action.productid)[0];
             indexofItem = state.cart.shoppingCartItems.indexOf(state.cart.shoppingCartItems
                                                         .filter(i => i.productid === action.productid)[0]);
